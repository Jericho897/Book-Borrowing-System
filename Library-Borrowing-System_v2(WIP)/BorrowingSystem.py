...
class AddBookDialog(QDialog):
    ...
    def get_book_info(self):
        ...
        try:
            ...
            Rentalfee = 0.0
        return BookId, Title, Author, Category, Status, Rentalfee, Description, Cover_image
...
class BookInfoDialog(QDialog):
    def __init__(self, book_info, bookId):
        ...
        delete_button.clicked.connect(lambda: self.delete_book(self.bookId))
    ...
    def open_update_dialog(self):
        update_dialog = UpdateBookDialog(self.book_info)
        if update_dialog.exec() == QDialog.DialogCode.Accepted:
            updated_book_info = update_dialog.get_updated_book_info()
            try:
                with sqlite3.connect("library.db") as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                        "UPDATE books SET Title = ?, Author = ?, Category = ?, Status = ?, RentalFee = ?, Description = ?, Cover_Image = ? WHERE BookId = ?",
                        (updated_book_info["Title"], updated_book_info["Author"], updated_book_info["Category"],
                         updated_book_info["Status"], updated_book_info["RentalFee"], updated_book_info["Description"],
                         updated_book_info["Cover Image"], self.bookId))
                    conn.commit()
    ...
    def delete_book(self, bookId):
        confirmation = QMessageBox.question(self, "Confirm Delete", "Are you sure you want to delete this book?",
        ...
        if confirmation == QMessageBox.Yes:
            try:
                with sqlite3.connect("library.db") as conn:
                    ...
                    conn.commit()
                    QMessageBox.information(self, "Delete Book", "Book deleted successfully")
                    self.accept()  # Close the dialog
            except sqlite3.Error as e:
...
class UpdateBookDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.book_id_label = QLabel(self.book_info["BookId"])
        form_layout.addRow(QLabel("Book ID:"), self.book_id_label)

        self.title_edit = QLineEdit(self.book_info["Title"])
        form_layout.addRow(QLabel("Title:"), self.title_edit)

        self.author_edit = QLineEdit(self.book_info["Author"])
        form_layout.addRow(QLabel("Author:"), self.author_edit)

        self.category_edit = QLineEdit(self.book_info["Category"])
        form_layout.addRow(QLabel("Category:"), self.category_edit)

        self.status_edit = QLineEdit(self.book_info["Status"])
        form_layout.addRow(QLabel("Status:"), self.status_edit)

        self.rental_fee_edit = QLineEdit(str(self.book_info["RentalFee"]))
        form_layout.addRow(QLabel("Rental Fee:"), self.rental_fee_edit)

        self.description_edit = QLineEdit(self.book_info["Description"])
        form_layout.addRow(QLabel("Description:"), self.description_edit)

        self.cover_image_edit = QLineEdit(self.book_info["Cover Image"])
        form_layout.addRow(QLabel("Cover Image:"), self.cover_image_edit)
    ...
    def get_updated_book_info(self):
        return {
            "BookId": self.book_info["BookId"],
            "Title": self.title_edit.text(),
            "Author": self.author_edit.text(),
            "Category": self.category_edit.text(),
            "Status": self.status_edit.text(),
            "RentalFee": float(self.rental_fee_edit.text()),
            "Description": self.description_edit.text(),
            "Cover Image": self.cover_image_edit.text(),
        }
...
class ActionDialog(QDialog):
    def __init__(self, book_info, bookId):
        ...
        return_button.clicked.connect(lambda: self.open_return_dialog(bookId))
    ...
    def get_rentalDueDate(self, book_id):
        try:
            # Create connection to database
            conn = sqlite3.connect("library.db")
            c = conn.cursor()

            # Fetch the rental due date from the database based on the book ID
            c.execute("SELECT RentalDueDate FROM rentals WHERE BookId = ?", (book_id,))
            rentalDueDate = c.fetchone()[0]

            conn.close()
            return rentalDueDate

        except sqlite3.Error as e:
            print("Error fetching rental due date:", e)
            return None

    def open_return_dialog(self, bookId):
        return_dialog = ReturnDialog()
        if return_dialog.exec() == QDialog.DialogCode.Accepted:
            customer_id, return_date = return_dialog.get_return_info()

            # Convert return_date to datetime object
            ...
            try:
                ...
                # Get the RentalDueDate and RentalFee from the rentals table
                c.execute("SELECT RentalDueDate FROM rentals WHERE BookId = ?", (bookId,))
                rentalDueDate = c.fetchone()
                if rentalDueDate is None:
                    print('Rental record not found')
                    QMessageBox.warning(self, "Error", "Rental record not found")
                    conn.close()
                    return

                rental_due_date = rentalDueDate[0]  # Extract the date string from the tuple
                rental_due_date = datetime.strptime(rental_due_date, '%Y-%m-%d %H:%M:%S')  # Convert to datetime object
                rental_due_date = rental_due_date.date()  # Extract the date part
                rental_fee = self.get_rental_fee(bookId)
                ...
                if return_date <= rental_due_date:
                    ...
                    days_overdue = (return_date - rental_due_date).days
                ...
                c.execute(
                    ...
                    (customer_id, bookId, return_date.strftime('%Y-%m-%d'), rental_due_date.strftime('%Y-%m-%d'),
                     overdue_fee))

                # Update the status of the book to 'Available'
                c.execute("UPDATE books SET Status = 'Available' WHERE BookId = ?", (bookId,))

                conn.commit()
                conn.close()
                QMessageBox.information(self, "Return Book",
                                        f"Returned by {customer_id} on {return_date.strftime('%Y-%m-%d')} with overdue fee {overdue_fee}")

            except sqlite3.Error as e:
                print('Error returning book:', e)
                QMessageBox.critical(self, "Error", f"Error returning book: {e}")

    def open_reserve_dialog(self, bookId):
        ...
        if reserve_dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            # Convert return_date to datetime object
            reserve_date = datetime.strptime(reservation_date, '%Y-%m-%d')
            ...
            try:
                ...
                # Check if the book is available
                c.execute("SELECT Status FROM books WHERE BookCode = ?", (bookId,))
                book_status = c.fetchone()
                if book_status is None:
                    QMessageBox.warning(self, "Error", "Book not found")
                    conn.close()
                ...
                c.execute(
                    ...
                    (customer_id, bookId, reservation_date, reservation_fee))

                # Update the status of the book to 'Reserved'
                c.execute("UPDATE books SET Status = 'Reserved' WHERE BookCode = ?", (bookId,))

                conn.commit()
                conn.close()
                QMessageBox.information(self, "Reserve Book",
                                        f"Reserved by {customer_id} on {reservation_date} with reservation fee {reservation_fee}")

            except sqlite3.Error as e:
                QMessageBox.critical(self, "Error", f"Error reserving book: {e}")
...
class ReturnDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.return_date_edit = QDateEdit(self)
        self.return_date_edit.setCalendarPopup(True)
        self.return_date_edit.setDisplayFormat("yyyy-MM-dd")
        self.return_date_edit.setDate(QDate.currentDate())  # Set initial date to today
    ...
    def get_return_info(self):
        ...
        return customer_id, return_date
        ...
        if selected_items:
            ...
            try:
                with sqlite3.connect("library.db") as conn:
                    ...
                    dialog = BookInfoDialog(book_info, book[0])
                    if dialog.exec() == QDialog.DialogCode.Accepted:
    ...
    def show_book_info_action(self):
        ...
        try:
            with sqlite3.connect("library.db") as conn:
                ...
                if dialog.exec() == QDialog.DialogCode.Accepted:
                    customer = dialog.get_rental_fee(book[0])  # wrong
        except sqlite3.Error as e:
            QMessageBox.critical(self, "Error", f"Error fetching book details: {e}")
